...
- Geofence-triggered start: when SPEC-024 fires an entry event for a job, create a time entry with `startTime = now, endTime = null`. On exit event, update the entry with `endTime = now`.
