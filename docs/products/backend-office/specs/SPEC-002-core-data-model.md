...
# BO-SPEC-002: Core Data Model

## Problem Statement

The app needs a new data model to support contractor profiles, contacts, jobs, estimates, invoices, and call logs. The existing schema (notes only) has been removed in BO-SPEC-001 and must be replaced with the core entities for Back-End Office.
...
6. **Audit logging owned by BO-SPEC-004** — the `auditLogs` table is not defined here. BO-SPEC-004 owns the full audit logging schema.
7. **SMS message tracking owned by BO-SPEC-012** — the `smsMessages` table is not defined here. BO-SPEC-012 owns SMS-related schema.
