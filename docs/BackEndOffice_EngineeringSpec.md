...
| **Job**        | id, contractor_id, contact_id, job_number, status (derived), description                                                                                                                        | Status (derived by `convex/lib/jobStatus.ts`): lead → estimated → approved → in_progress → completed → paid. Scheduling lives on segments, not jobs. |
| **JobSegment** | id, job_id, contractor_id, sequence, title, status, scheduled_at, completed_at                                                                                                                  | A job is split into one or more segments. Single-visit jobs have one segment. Status: scheduled → in_progress → completed (or canceled).             |
| **Estimate**   | id, job_id, estimate_number, version, line_items (JSON, each with optional segment_id/segment_title), total, status, sent_at, approved_at                                                       | Line items: \[{description, qty, unit_price, total, type, segment_id?, segment_title?}\]. Approval auto-creates segments from line item groupings.   |
| **Invoice**    | id, job_id, estimate_id, invoice_number, invoice_type (deposit/progress/final), segment_ids\[\], credited_deposit_invoice_ids\[\], line_items (JSON), total, status, paid_at, stripe_payment_id | A job can have many invoices. Deposit credits applied as snapshot negative line items.                                                               |
